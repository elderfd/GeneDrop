...
void NewPedigree::addCross(std::string fatherName, std::string motherName, std::string childName) {
	// Work out if the mother and father already exist within the pedigree
	class IndividualFinder : public NodeChecker {
	public:
		std::string name;
		bool operator()(std::shared_ptr<NewPedigreeNode> node) {
	...
	// Create the new node
	auto newNode = std::make_shared<NewPedigreeNode>();
...
std::vector<std::shared_ptr<NewPedigreeNode>> NewPedigree::depthFirstSearchManyNodes(bool(*predicate)(std::shared_ptr<NewPedigreeNode>)) {
	std::vector<std::shared_ptr<NewPedigreeNode>> returnNodes;

	std::function<
		void(std::shared_ptr<NewPedigreeNode>, bool(std::shared_ptr<NewPedigreeNode>), std::vector<std::shared_ptr<NewPedigreeNode>>*)
	> visitNode = [&visitNode](std::shared_ptr<NewPedigreeNode> node, bool(*predicate)(std::shared_ptr<NewPedigreeNode>), std::vector<std::shared_ptr<NewPedigreeNode>>* nodes) {
		if (predicate(node)) {
...
std::vector<std::shared_ptr<NewPedigreeNode>> NewPedigree::depthFirstSearchManyNodes(NodeChecker* predicate) {
	std::vector<std::shared_ptr<NewPedigreeNode>> returnNodes;

	std::function<void(std::shared_ptr<NewPedigreeNode>, NodeChecker*, std::vector<std::shared_ptr<NewPedigreeNode>>*)> visitNode = [&visitNode](std::shared_ptr<NewPedigreeNode> node, NodeChecker* predicate, std::vector<std::shared_ptr<NewPedigreeNode>>* nodes) {
		if ((*predicate)(node)) {
			if (std::find(nodes->begin(), nodes->end(), node) == nodes->end()) nodes->push_back(node);
		}
...
std::shared_ptr<NewPedigreeNode> NewPedigree::depthFirstSearchSingleNode(NodeChecker* predicate) {
	std::shared_ptr<NewPedigreeNode> returnNode = nullptr;

	std::function<std::shared_ptr<NewPedigreeNode>(std::shared_ptr<NewPedigreeNode>, NodeChecker*)> visitNode = [&visitNode](std::shared_ptr<NewPedigreeNode> node, NodeChecker* predicate) {
		if ((*predicate)(node)) {
			return node;
		}
		...
		return std::shared_ptr<NewPedigreeNode>();
	};
...
std::shared_ptr<NewPedigreeNode> NewPedigree::depthFirstSearchSingleNode(bool(*predicate)(std::shared_ptr<NewPedigreeNode>)) {
	std::shared_ptr<NewPedigreeNode> returnNode = nullptr;
	
	std::function<
		std::shared_ptr<NewPedigreeNode>(std::shared_ptr<NewPedigreeNode>, bool(std::shared_ptr<NewPedigreeNode>))
	> visitNode = [&visitNode](std::shared_ptr<NewPedigreeNode> node, bool(*predicate)(std::shared_ptr<NewPedigreeNode>)) {
		if (predicate(node)) {
			return node;
		}
...
void NewPedigree::updateCrossOrder() {
	crossOrder.clear();
	...
	std::vector<std::shared_ptr<NewPedigreeNode>> nodesWithNoIncomingEdges = leaves;;
	...
	crossOrder.erase(std::remove_if(crossOrder.begin(), crossOrder.end(), [](const std::shared_ptr<NewPedigreeNode>& ptr) {
		return ptr == nullptr;
	}), crossOrder.end());
...
CrossIterator NewPedigree::begin() {
...
CrossIterator NewPedigree::end() const {
...
CrossIterator::CrossIterator(const NewPedigree* pedigree) :
	pedigree(pedigree) {
